...
const folderView	 = __path_view_ecommerce + 'pages/search/';
...
router.get('/',async (req, res, next) => {
  ...
  const listProducts = await productsModel.find({name: {$regex: keyword}});
  const listProductBestSeller = await productsModel.find({bestseller:true,status:'active', name: {$regex: keyword}});
  const listProductNewArrivals = await productsModel.find({newarrivals:true,status:'active', name: {$regex: keyword}});
