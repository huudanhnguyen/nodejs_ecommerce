...
const blogModel 		= require(__path_models + col_blogArticle);
const blogSchemas 		= require(__path_schemas + col_blogArticle);
const Model 		= require(__path_models + 'products');
const menuModel 		= require(__path_schemas + col_menu);
const folderView	 = __path_view_ecommerce + 'pages/blogArticle/';
...
router.get("/:id",async (req, res, next) => {
  ...
  let idBlog = ParamsHelpers.getParam(req.params, 'id', '');
  let itemBlog = {};
  await blogModel.getItemFrontend(idBlog, null).then((itemBlg) => {
    itemBlog = itemBlg;
  });

  const listProducts = await productsModel.find({});
  const listBlog = await blogSchemas.find({status:'active'});
